...
User findUser(XLWorksheet &sheet, const std::string &username)
{
    for (int row = 2; sheet.cell(row, 1).value().type() != XLValueType::Empty; ++row)
    {
        if (sheet.cell(row, 2).value().get<std::string>() == username)
        {
            return {
                username,
                sheet.cell(row, 3).value().type() == XLValueType::String ? sheet.cell(row, 3).value().get<std::string>() : "", // Nếu rỗng, gán ""
                ...
                row,
                sheet.cell(row, 13).value().type() == XLValueType::Integer ? (sheet.cell(row, 13).value().get<int>() == 1) : false};
        }
    }
    return {"", "", "", 0, 0, false};
}
...
{
    if (logSheet.rowCount() == 0)
    { // Kiểm tra xem sheet có dữ liệu không
        cout << "Lỗi: Sheet log giao dịch không hợp lệ hoặc trống!" << endl;
        return;
    }
    ...
    // ID giao dịch = số dòng - 1 (bỏ dòng tiêu đề)
    ...
    logSheet.cell(newRow, 6).value() = senderId - 1;
    logSheet.cell(newRow, 7).value() = recipientId - 1;
...
void transferPoints(const std::string &filePathUser, const std::string &filePathLog, const std::string &userNameSender)
{
    ...
    bool hasAdmin = false;
    for (int row = 2; row <= userSheet.rowCount(); ++row)
    { // Bắt đầu từ hàng 2 (bỏ qua tiêu đề)
        if (userSheet.cell(row, 13).value().get<int>() == 1)
        { // Cột M = index 13
            hasAdmin = true;
            break;
        }
    ...
    if (recipientName == sender.username)
    {
        cout << "❌ Không thể chuyển điểm cho chính mình!\n";
        return;
    }
    ...
    // Nếu người gửi là client và muốn chuyển nhiều hơn số điểm của admin → không cho phép
    ...
    if (!sender.is_admin)
    {
        ...
        otp.sendOTPToUser(sender.username); // gửi người dùng
    ...
    userSheet.cell(sender.user_id, 12).value() = "Chuyển điểm thành công";

    userSheet.cell(recipient.user_id, 11).value() = recipient.balance;
    userSheet.cell(recipient.user_id, 12).value() = "Nhận điểm";

    // Ghi log giao dịch
    logTransaction(logSheet, sender.user_id, "Chuyển điểm thành công", oldSenderBalance, sender.balance, sender.user_id, recipient.user_id);
    logTransaction(logSheet, recipient.user_id, "Nhận điểm", oldRecipientBalance, recipient.balance, sender.user_id, recipient.user_id);
